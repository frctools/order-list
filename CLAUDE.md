...
Some vendors (Online Metals) sit behind a bot challenge that answers a Worker's fetch with an interstitial, so `extract.get.ts` sends those hosts to vendord *first* via `server/utils/vendord.ts` (`DELEGATED_HOSTS`), mapping the scraper's reply back into the extractor's own result shape. If vendord is down or blocked in turn, the extractor's fallbacks still run — including a last-resort parse of the URL itself (`source: 'url'`), which fills in a name and SKU but no price.
